...
class SoftKTreesEnsembleGate(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        super(SoftKTreesEnsembleGate, self).__init__()
...
#         assert self.nb_experts == 2**self.max_depth # to check number of experts is a power of 2 
        ...
        self.balanced_splitting_penalty = config.get("balanced_splitting_penalty", 0.)
        self.exp_decay_mov_ave = config.get("exp_decay_mov_ave", 0.)
        ...
        if not self.use_routing_input:
            if not self.leaf:
                ...
                self.left_child = SoftKTreesEnsembleGate(config, 2*self.node_index+1, depth_index=self.depth_index+1, name="Node-"+str(2*self.node_index+1))
                self.right_child = SoftKTreesEnsembleGate(config, 2*self.node_index+2, depth_index=self.depth_index+1, name="Node-"+str(2*self.node_index+2))
            else:
...
#         return s_bj, d_bj
    ...
    def _compute_balanced_split_loss(self, prob, current_prob):
        ...
        alpha_ave_current = tf.reduce_sum(
            prob*current_prob,
            axis=0
        )/(
            tf.reduce_sum(
                prob*tf.ones_like(current_prob),
                axis=0
            ) + EPSILON
        )
        alpha_ave = (1-exp_decay_mov_ave)*tf.cast(alpha_ave_current, dtype=self.dtype) + exp_decay_mov_ave*tf.cast(self.alpha_ave_past, dtype=self.dtype)
        self.alpha_ave_past.assign(alpha_ave)
        
        loss = -penalty*(0.5*tf.math.log(alpha_ave + EPSILON)+0.5*tf.math.log(1-alpha_ave + EPSILON))
        loss = tf.reduce_mean(loss, axis=-1)
        loss = tf.reduce_sum(loss)
#         tf.print("===========bal-split loss:", loss)
        self.add_metric(loss, name="load_balancing_loss")
        return loss
...
#                 s_bj_sp = tf.concat([s_left_child_sp, s_right_child_sp], axis=-1)  
                ...
                if self.balanced_splitting:
                    load_balancing_loss = control_flow_util.smart_cond(
                        ...
                        lambda: tf.cond(
                            tf.math.equal(
                                tf.cast(self.balanced_splitting_penalty, self.dtype),
                                tf.constant(0.0, dtype=self.dtype)
                            ),
                            lambda: tf.zeros(()),
                            lambda: self._compute_balanced_split_loss(prob, current_prob)                            
                        ),
                        lambda: tf.zeros(())
                    )
...
class SoftKTreesEnsembleGateOldV5(tf.keras.layers.Layer):
    """An ensemble of soft decision trees.
...
#         assert self.nb_experts == 2**self.max_depth # to check number of experts is a power of 2 
        ...
        if not self.use_routing_input:
            if not self.leaf:
                ...
                self.left_child = SoftKTreesEnsembleGate(config, 2*self.node_index+1, name="Node-"+str(2*self.node_index+1))
                self.right_child = SoftKTreesEnsembleGate(config, 2*self.node_index+2, name="Node-"+str(2*self.node_index+2))
            else:
                masking = np.zeros((1, 1, self.nb_experts))
#                 tf.print("=========self.nb_experts:", self.nb_experts)
#                 tf.print("=========self.node_index:", self.node_index)
...
#         return s_bj, d_bj
    ...
    def call(self, inputs, training=True, prob=1.0):
...
#                 s_bj_sp = tf.concat([s_left_child_sp, s_right_child_sp], axis=-1)            
                if self.node_index==0:
                    w_concat = tf.nn.softmax(s_bj, axis=-1)
#                     w_concat_sp = tf.nn.softmax(s_bj_sp,axis=-1)
#                     tf.print("=== w_diff", tf.reduce_max(tf.math.abs(w_concat-w_concat_sp)))
                    w_concat = tf.expand_dims(w_concat, axis=1) # (b, 1, nb_experts)
#                     w_concat = w_concat/tf.reduce_sum(w_concat, axis=-1, keepdims=True)  # (b, 1, nb_experts)
                    # h:(b, dim_exp_i, nb_experts) * w_concat: (b, 1, nb_experts)
                    y_agg = tf.reduce_sum(h * w_concat, axis=-1) # (b, dim_exp_i, nb_experts) -> (b, dim_exp_i)
                    ...
                    soft_averages = tf.reduce_mean(w_concat, axis=[0,1]) # (nb_experts,)
                    hard_averages = tf.reduce_mean(tf.ones_like(s_concat)-s_concat, axis=[0,1]) # (nb_experts,)
                    soft_averages_for_all_experts_list = tf.split(
                        tf.reshape(soft_averages, [-1]),
                        self.nb_experts
                    )
                ...
                log_prob = tf.where(
                    tf.math.equal(prob, tf.constant(0.0)),
                    (tf.experimental.numpy.finfo(tf.float32).min)*tf.ones_like(prob),              
                    # tf.math.log(prob+tf.experimental.numpy.finfo(tf.float32).eps)
                    tf.math.log(prob+1e-6)
                )
                s_bj = tf.reduce_logsumexp(a_bij+log_prob, axis=-1, keepdims=True) # (b, 1)
#                 s_bj_sp = tf.reduce_logsumexp(a_bij+tf.math.log(prob),axis=-1,keepdims=True)
                regularization_per_expert = control_flow_util.smart_cond(
...
class SoftKTreesEnsembleGateOldV4(tf.keras.layers.Layer):
    """An ensemble of soft decision trees.
    ...
    def __init__(self,
        ...
        self.use_routing_input = config["use_routing_input"]
        self.nb_experts = config["nb_experts"]
        self.max_depth = (int)(np.log2(self.nb_experts))
        self.k = config["k"]
#         tf.print("=========self.nb_experts:", self.nb_experts)
#         tf.print("=========self.max_depth:", self.max_depth)
        self.node_index = node_index
...
#                 tf.print("======f_bj.shape: ", f_bj.shape)
            ...
            if not self.leaf:
                # shape = (batch_size, k)
                current_prob = self.selector_layer(x) # (batch_size, k)
                f_left_child, w_left_child, a_bi_max_left_child = self.left_child(inputs, training=training, prob=current_prob * prob)
                f_right_child, w_right_child, a_bi_max_right_child = self.right_child(inputs, training=training, prob=(1 - current_prob) * prob)
                a_bi_max = tf.math.maximum(a_bi_max_left_child, a_bi_max_right_child)
                f_agg = f_left_child*tf.math.exp(a_bi_max_left_child-a_bi_max) + f_right_child*tf.math.exp(a_bi_max_right_child-a_bi_max)
                w_concat = tf.concat([
                    w_left_child*tf.expand_dims(tf.math.exp(a_bi_max_left_child-a_bi_max), axis=1),
                    w_right_child*tf.expand_dims(tf.math.exp(a_bi_max_right_child-a_bi_max), axis=1)
                ], axis=-1)
                if self.node_index==0:
                    w_agg = tf.reduce_sum(w_concat, axis=-1)
                    y_agg = f_agg/w_agg
...
#                     tf.print("======s_avg.shape:", s_avg.shape)
                    ...
                    return f_agg, w_concat, a_bi_max
            else:
                # prob's shape = (b, k)
                # Computing a_bij,    a_bij shape = (b, k)
                a_bij = self.output_layer(x) # (b, k) # Note we do not have access to j as j represents leaves
                a_bi_max = tf.math.reduce_max(a_bij, axis=1, keepdims=True) # (b, 1)
                a_exp_bij = tf.math.exp(a_bij-a_bi_max)
                r_bij = prob * a_exp_bij # (b, k)
                r_bij = tf.expand_dims(r_bij, axis=1) # (b, 1, k)
                
                # Computing w_bj, 
                w_bj = tf.reduce_sum(r_bij, axis=-1) # (b, 1, k) -> (b, 1)
                w_bj = tf.expand_dims(w_bj, axis=-1) # (b, 1, 1)
#                 tf.print("======w_bj.shape: ", w_bj.shape)
...
#                 tf.print("======f_bj.shape: ", f_bj.shape)
                ...
                return f_bj, w_bj, a_bi_max
            
            
class SoftKTreesEnsembleGateOldV3(tf.keras.layers.Layer):
    """An ensemble of soft decision trees.
...
class SoftKTreesEnsembleGateOldV2(tf.keras.layers.Layer):
    """An ensemble of soft decision trees.
    ...
    def __init__(self,
        ...
        super(SoftKTreesEnsembleGateOldV2, self).__init__()
        self.use_routing_input = config["use_routing_input"]
        self.nb_experts = config["nb_experts"]
        self.max_depth = (int)(np.log2(self.nb_experts))
        self.k = config["k"]
...
#         tf.print("=========self.node_index:", self.node_index)
        ...
        if not self.use_routing_input:
            ...
            if not self.leaf:
                ...
                self.left_child = SoftKTreesEnsembleGateOldV2(config, 2*self.node_index+1, name="Node-"+str(2*self.node_index+1))
                self.right_child = SoftKTreesEnsembleGateOldV2(config, 2*self.node_index+2, name="Node-"+str(2*self.node_index+2))
            else:
                masking = np.zeros((1, 1, self.nb_experts))
#                 tf.print("=========self.nb_experts:", self.nb_experts)
#                 tf.print("=========self.node_index:", self.node_index)
    ...
    def call(self, inputs, training=True, prob=1.0):
        ...
        if not self.use_routing_input:
            ...
            if not self.leaf:
                # shape = (batch_size, k)
                current_prob = self.selector_layer(x) # (batch_size, k)
                f_left_child, w_left_child, s_left_child = self.left_child(inputs, training=training, prob=current_prob * prob)
                f_right_child, w_right_child, s_right_child = self.right_child(inputs, training=training, prob=(1 - current_prob) * prob)
                f_agg = f_left_child + f_right_child
                w_agg = w_left_child + w_right_child
                s_agg = s_left_child + s_left_child
                if self.node_index==0:
                    y_agg = f_agg/w_agg
                    s_agg_avg = s_agg / tf.cast(self.nb_experts, dtype=self.dtype)
                    avg_sparsity = tf.reduce_mean(s_agg_avg) # average over batch
                    self.add_metric(
                        avg_sparsity,
                        name='avg_sparsity'
                    )    
...
#                 tf.print("======f_bj.shape: ", f_bj.shape)
                ...
                # Compute s_bj
                s_bj = tf.where(
                    tf.math.less(w_bj, 1e-5),
                    tf.ones_like(w_bj),
                    tf.zeros_like(w_bj)
                ) # (b, 1)
#                 tf.print("======s_bj.shape: ", s_bj.shape)
                ...
                return f_bj, w_bj, s_bj

class SoftKTreesEnsembleGateOld(tf.keras.layers.Layer):
    """An ensemble of soft decision trees.
    ...
    def call(self, inputs, training=True, prob=1.0):
        ...
        if not self.use_routing_input:
            ...
            if not self.leaf:
                ...
                y_left_child, sparsity_left = self.left_child(inputs, training=training, prob=current_prob * prob)
                y_right_child, sparsity_right = self.right_child(inputs, training=training, prob=(1 - current_prob) * prob)
                y = y_left_child + y_right_child
                sparsity = sparsity_left + sparsity_left
                if self.node_index==0:
                    avg_sparsity = sparsity / tf.cast(self.nb_experts, dtype=self.dtype)
                    self.add_metric(
                        avg_sparsity,
                        name='avg_sparsity'
                    )    
                ...
                # prob's shape = (batch_size, k)
                # selector_weights shape = (batch_size, k)
                selector_weights = self.output_layer(x) # (batch_size, k)
                selector_weights = tf.nn.softmax(selector_weights, axis=-1) # (batch_size, k)
                output = prob * selector_weights # (batch_size, k)
                output = tf.reduce_sum(output, axis=-1, keepdims=True) # (batch_size, 1)
                # h:  (bs, dim_exp_i, nb_experts),   self.masking: (1, 1, self.nb_experts)
                y = output * tf.reduce_sum(h * self.masking, axis=-1) # (bs, dim_exp_i)
#                 tf.print("======y.shape: ", y.shape)

                sparsity = tf.where(
                    tf.math.less(output, 1e-5),
                    tf.ones_like(output),
                    tf.zeros_like(output)
                )
                sparsity_per_expert = tf.reduce_mean(sparsity)
...
if __name__ == "__main__":
    ...
    s = SoftKTreesEnsembleGate(config)
    h = [
